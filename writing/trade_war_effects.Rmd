...
While evaluating a question of how a policy would change the approval rate of one party, there is no cross-sectional variation in treatment-only one party is observed at a given time, so no difference-in-differences is possible. However, it is possible to estimate the impact under a regression discontinuity (RD) context. In this study, we used regression discontinuity in time (RDiT) to measure the trade war's effect on Donald Trump's approval rate. Different from cross-sectional RD, the running variable is time itself in the RDiT framework, and time cannot be thought of as randomly assigned within a neighborhood around a threshold like RD framework assigning randomized experiment [@2018RDiT].
